...
router.post("/:id/buy", requireAuth, async (req, res, next) => {
  try {
    ...
    await container.items.commands.buyAsync.execute(new BuyItemCommand(itemId, req.user.userId));
    res.status(204).send();
  } catch (e) {
    next(e);
  }
...
router.post("/:id/buy-sync", requireAuth, async (req, res, next) => {
  try {
    if (!req.user) throw new AuthenticationError("Unauthorized");
    const itemId = parseId(String(req.params.id), "item");
    await container.items.commands.buySync.execute(new BuyItemCommand(itemId, req.user.userId));
    res.status(204).send();
  } catch (e) {
    next(e);
  }
...
router.post("/:id/buy-async", requireAuth, async (req, res, next) => {
  try {
    if (!req.user) throw new AuthenticationError("Unauthorized");
    const itemId = parseId(String(req.params.id), "item");
    await container.items.commands.buyAsync.execute(new BuyItemCommand(itemId, req.user.userId));
