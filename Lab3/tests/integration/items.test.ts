...
describe("Items API", () => {
  ...
  describe("POST /items", () => {
    ...
    it("should allow Admin to create item", async () => {
      ...
      const created = await request(app)
        .get(`/items/${res.body.id}`)
        .set("Authorization", `Bearer ${adminToken}`);

      expect(created.status).toBe(200);
      expect(created.body.OwnerID).toBe(adminId);
      expect(created.body.Price).toBe(500);
    });
  ...
  describe("POST /items/:id/buy", () => {
    ...
    it("should allow user to buy item", async () => {
      ...
      const itemAfterBuy = await request(app)
        .get(`/items/${itemId}`)
        .set("Authorization", `Bearer ${buyerToken}`);
      expect(itemAfterBuy.status).toBe(200);
      expect(itemAfterBuy.body.OwnerID).toBe(buyerId);

      const buyerInDb = await prisma.users.findUnique({ where: { ID: buyerId } });
      const sellerInDb = await prisma.users.findUnique({ where: { ID: adminId } });

      expect(buyerInDb?.Balance).toBe(700);
